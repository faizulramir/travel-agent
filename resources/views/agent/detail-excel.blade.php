                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-3" style="text-align: left;">
                            <a href="{{ route('excel_list_agent') }}" class="btn btn-primary w-md">
                                <i class="bx bx-chevrons-left font-size-24" title="Back"></i>
                            </a>
                        </div>
                        <div class="col-md-3 text-right">
                            <h6>TravelAgent Name: {{ $uploads->ta_name? strtoupper($uploads->ta_name) : $uploads->ta_name }}</h6>
                        ...
                        <div class="col-md-6" style="text-align: right;">
                            ...
                            <div class="col-md-6" style="text-align: right; display: {{ count($check) != 0 ? 'inline' : 'none' }}">
                                <button class="btn btn-primary w-md" id="download_all_cert" onclick="downloadall_ecert({{$uploads->id}})" title="Download all ECert">Download All ECert</button>
                            </div>
                            @endif
                        </div>                        
                    </div>
...
@section('script')
    ...
    <script>
        ...
        function openDetail (id, docs) {
            ...
            if (docs) {
                $.ajax({
                    url: '/supp_doc_check_agn/' + id + '/' + docs,
                    type: 'GET',
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function (data) {
                        ...
                        if (docs.includes('P') && objp!=null && objp!=undefined) {
                            $('#passportdownload').html('<a target="_blank" href="/supp_doc_download_agn/' + id + '/passport" type="submit">'+ objp.passport +'</a>');
                        } else {
                            $('#passportdownload').html('');
                        }

                        if (docs.includes('T') && obje!=null && obje!=undefined) {
                            $('#eticketdownload').html('<a target="_blank" href="/supp_doc_download_agn/' + id + '/eticket" type="submit">'+ obje.eticket +'</a>');
                        } else {
                            $('#eticketdownload').html('');
                        }

                        if (docs.includes('V') && objv!=null && objv!=undefined) {
                            $('#visadownload').html('<a target="_blank" href="/supp_doc_download_agn/' + id + '/visa" type="submit">'+ objv.visa +'</a>');
                        } else {
                            $('#visadownload').html('');
                        }

                        if (docs.includes('R') && objr!=null && objr!=undefined) {
                            $('#payreceiptdownload').html('<a target="_blank" href="/supp_doc_download_agn/' + id + '/payreceipt" type="submit">'+ objr.payreceipt +'</a>');
                        } else {
