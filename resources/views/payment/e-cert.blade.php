<html>
    ...
    <body style="background-image: url({{ $url_bg }}); background-repeat: no-repeat; background-attachment: fixed;  background-size: cover;">
        ...
        @php
            $ta_name = '';
            if ($orders->upload->user->hasAnyRole('ind')) {
                $ta_name = 'DIY';
            } else {
                $ta_name = $orders->upload->ta_name;
            }
        @endphp
        <div class="row">
            <div class="column-second" style="width: 60%;">
                <p style="padding-top: {{ strlen($ta_name) > 27 ? '14px' : '10px' }}; font-size: {{ strlen($ta_name) > 27 ? '18px' : '22px' }};"><b>{{ strtoupper($ta_name) }}</b></p>
